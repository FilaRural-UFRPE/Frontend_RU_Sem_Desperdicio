...
export default function WinnerPage() {
  ...
  const selectWinner = async (entry) => {
    if (!entry?.cpf) return
    const replaceMsg = winner ? ` Isso substituirá o vencedor atual (${winner.name}).` : ''
    const confirmed = window.confirm(
      `Registrar ${entry.name} (CPF ${entry.cpf}) como vencedor de ${monthLabel}?${replaceMsg}\n\nEsta ação é definitiva.`
    )
    if (!confirmed) return
    setSaving(true)
    try {
      await rankingAPI.setWinner(year, month, entry.cpf)
  ...
  const drawRaffle = async () => {
    ...
    try {
      ...
      const createRes = await rankingAPI.raffleCreate(
        `Sorteio ${monthLabel}`,
        startDate,
        endDate,
        1
      ...
      toast(
        `Sorteio realizado! ${drawRes.data.winners.length} vencedor(es) de ${drawRes.data.total_participants} participantes.`
      )
      await load()
    } catch (error) {
      toast(
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-ru-muted mt-2">
            Sorteie aleatoriamente ou selecione manualmente o vencedor.
          </p>
        </div>
      ...
      {winner && (
        <section className="card bg-gradient-to-r from-emerald-50 to-green-50 border-2 border-emerald-200 flex items-center gap-4">
          ...
          <div>
            <p className="font-mono text-xs uppercase tracking-widest text-emerald-700 font-semibold">
              Vencedor registrado
            </p>
      ...
      <section className="card">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-xl font-semibold flex items-center gap-2">
            <Crown size={20} className="text-ru-yellow" /> Ranking de {monthLabel}
          </h2>
          <button
            onClick={drawRaffle}
        ...
        {drawResult && (
          <div className="mb-4 rounded-xl bg-emerald-50 border border-emerald-200 p-4">
            ...
            <p className="text-sm text-ru-muted mt-1">
              {drawResult.winners.length} vencedor(es) sorteado(s) entre{' '}
              {drawResult.total_participants} participantes.
            </p>
            ...
            <p className="text-xs text-ru-muted mt-2">
              Os vencedores foram notificados por push e email.
            </p>
          </div>
        )}
            ...
            <table className="responsive-table w-full text-sm">
              ...
              <tbody>
                {!rankData?.data?.length ? (
                  <tr className="empty">
                    <td colSpan="6" className="py-10 text-center text-ru-muted" data-label="">
                  ...
                  rankData.data.map((entry) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-ru-cream/60 ${isWinner ? 'bg-emerald-50' : ''}`}
                      >
                        <td className="py-3 pr-3 font-display text-lg" data-label="Posição">
                          {medal(entry.rank)}{' '}
                        ...
                        <td className="py-3 pr-3 text-ru-charcoal" data-label="Estudante">
                          {entry.name}
                          {isWinner && (
                            <CheckCircle2 size={14} className="inline text-emerald-500 ml-1" />
                          )}
                        </td>
                        ...
                        <td
                          className="py-3 pr-3 text-center font-mono font-bold text-emerald-600"
