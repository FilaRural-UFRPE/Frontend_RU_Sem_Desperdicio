    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={mes}
            onChange={(e) => { setPage(1); setMes(e.target.value) }}
      ...
      {winner && (
        <section className="card bg-gradient-to-r from-ru-yellow/20 to-amber-50 border-2 border-ru-yellow/40 flex items-center gap-4">
          ...
          <div>
            <p className="font-mono text-xs uppercase tracking-widest text-amber-600 font-semibold">Vencedor do mês</p>
      ...
      <section className="card">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display text-xl font-semibold flex items-center gap-2"><Users size={20} className="text-ru-blue" /> {monthLabel}</h2>
          {!error && <span className="tag bg-blue-50 text-ru-blue">{data?.total ?? '—'} participantes</span>}
        </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="responsive-table w-full text-sm">
              ...
              <tbody>
                {!data?.data?.length ? (
                  <tr className="empty"><td colSpan="5" className="py-10 text-center text-ru-muted" data-label="">Nenhum dado para este mês.</td></tr>
                ) : data.data.map((entry) => {
                  const isMe = user && `${entry.name}` === user.name
                  return (
                    <tr key={`${entry.rank}-${entry.name}`} className={`border-b border-ru-cream/60 ${isMe ? 'bg-ru-blue/5 font-semibold' : ''}`}>
                      <td className="py-3 pr-3 font-display text-lg text-ru-charcoal" data-label="Posição">{medal(entry.rank)} <span className="font-mono text-sm">{entry.rank}</span></td>
                      <td className="py-3 pr-3 text-ru-charcoal" data-label="Estudante">{entry.name}{isMe && <span className="ml-2 text-xs text-ru-blue">(você)</span>}</td>
                      ...
                      <td className="py-3 text-center font-mono font-bold text-emerald-600" data-label="Comparecimentos">{entry.confirmed_count}</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-5 pt-4 border-t border-ru-cream-dark">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="btn-secondary px-3 py-2 inline-flex items-center gap-2 disabled:opacity-40"><ChevronLeft size={16} /> Anterior</button>
            <span className="text-sm text-ru-muted">Página {page} de {totalPages}</span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="btn-secondary px-3 py-2 inline-flex items-center gap-2 disabled:opacity-40">Próxima <ChevronRight size={16} /></button>
          </div>
        )}
      </section>
    </div>
