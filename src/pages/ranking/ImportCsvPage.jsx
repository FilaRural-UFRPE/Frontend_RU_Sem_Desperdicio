...
export default function ImportCsvPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast(error.response?.data?.detail?.msg || error.response?.data?.msg || 'Não foi possível importar o CSV', 'error')
    } finally {
      setBusy(false)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <header>
        <p className="font-mono text-xs uppercase tracking-[0.18em] text-ru-blue">Secretaria · Comparecimento ao RU</p>
        <h1 className="font-display text-3xl md:text-4xl font-bold mt-2">Importar presenças</h1>
        <p className="text-ru-muted mt-2">Envie o CSV da secretaria com os comparecimentos do dia. O ranking é atualizado na hora.</p>
      </header>
      ...
      <section className="card">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <button onClick={downloadTemplate} className="btn-secondary inline-flex items-center gap-2"><Download size={16} /> Baixar modelo</button>
        </div>
        <ul className="mt-4 text-xs text-ru-muted space-y-1">
          <li>· Colunas: <code className="font-mono">cpf</code>, <code className="font-mono">date</code> (DD/MM/AAAA) e <code className="font-mono">meal_type</code> (lunch/almoco, dinner/jantar)</li>
      ...
      <section className="card">
        <div className="flex flex-col sm:flex-row items-stretch gap-3">
          <label className={`flex-1 flex flex-col items-center justify-center border-2 border-dashed rounded-2xl py-8 px-4 cursor-pointer transition-colors ${file ? 'border-emerald-400 bg-emerald-50/50' : 'border-ru-cream-dark hover:border-ru-blue'}`}>
            <UploadCloud size={28} className={file ? 'text-emerald-500' : 'text-ru-muted'} />
            <p className="font-display font-semibold mt-3">{file ? file.name : 'Selecionar arquivo CSV'}</p>
            <p className="text-xs text-ru-muted mt-1">{file ? `${(file.size / 1024).toFixed(1)} KB` : 'ou arraste e solte aqui'}</p>
            <input ref={fileRef} type="file" accept=".csv,text/csv" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          </label>
        </div>
        <button onClick={submit} disabled={!file || busy} className="btn-primary w-full mt-4 inline-flex justify-center items-center gap-2 disabled:opacity-50">
        ...
        <section className="card">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-emerald-50 rounded-xl p-4 text-center">
              ...
              <p className="font-display text-3xl font-bold text-emerald-600 mt-2">{result.accepted}</p>
            ...
            <div className="bg-amber-50 rounded-xl p-4 text-center">
              ...
              <p className="font-display text-3xl font-bold text-amber-600 mt-2">{result.rejected}</p>
              <p className="text-xs text-ru-muted mt-1">Linhas ignoradas</p>
            </div>
          </div>
          {result.errors?.length > 0 && (
            <div className="mt-4">
              <p className="text-xs font-semibold text-ru-muted uppercase tracking-wide mb-2">Detalhes das linhas ignoradas</p>
              <div className="max-h-48 overflow-y-auto rounded-xl bg-ru-cream/60 divide-y divide-ru-cream-dark">
                {result.errors.map((err, i) => (
                  <div key={i} className="px-4 py-2 text-xs">
                    ...
                    <span className="text-ru-muted ml-2 font-mono">{JSON.stringify(err.row)}</span>
                  </div>
                ))}
              </div>
            </div>
