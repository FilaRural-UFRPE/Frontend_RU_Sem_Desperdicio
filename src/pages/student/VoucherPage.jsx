    ...
    <div className="max-w-5xl mx-auto space-y-6 voucher-page">
      ...
      <section className="card overflow-hidden relative">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 text-ru-blue font-display font-semibold"><Sparkles size={18} /> Minha premiação</div>
        ...
        {eventVoucher ? (
          <div className="voucher-ticket bg-ru-blue-dark text-white rounded-3xl overflow-hidden shadow-xl grid md:grid-cols-[1fr_auto] mt-4">
            <div className="p-7 md:p-8 flex flex-col justify-between">
              <div>
                <span className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-blue-100"><ShieldCheck size={16} /> QR code de evento</span>
                <h3 className="font-display text-2xl font-bold mt-4">5 almoços no RU.</h3>
                <p className="text-blue-100 mt-2 max-w-md text-sm">Você foi premiado na roleta da campanha. Apresente este QR no caixa a cada almoço.</p>
              </div>
              <div className="mt-6 pt-4 border-t border-white/20 flex flex-wrap gap-x-8 gap-y-3 text-sm">
                <div><p className="text-blue-200 text-xs uppercase tracking-wide">Titular</p><p className="font-semibold mt-1">{user?.name}</p></div>
                <div><p className="text-blue-200 text-xs uppercase tracking-wide">Código</p><p className="font-mono font-semibold mt-1">{eventVoucher.code}</p></div>
                <div><p className="text-blue-200 text-xs uppercase tracking-wide">Refeições</p><p className="font-semibold mt-1">{eventVoucher.meals_used} / {eventVoucher.total_meals}</p></div>
              </div>
                ...
                <div className="mt-4 pt-4 border-t border-white/20">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {eventVoucher.usages.map((u) => (
                      <span key={u.id} className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-xs">
          ...
          <div className="card text-center py-10 mt-4">
            ...
            <p className="text-ru-muted text-sm mt-1">Complete 10 dias de agendamento e presença para concorrer na roleta da campanha.</p>
          </div>
        )}
      </section>
    </div>
