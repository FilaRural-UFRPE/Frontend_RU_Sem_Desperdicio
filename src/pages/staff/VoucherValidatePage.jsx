...
export default function VoucherValidatePage() {
  ...
  const syncPending = useCallback(async () => {
    ...
    try {
      const { data } = await voucherAPI.sync(
        usages.map(({ voucher_id, used_at }) => ({ voucher_id, used_at }))
      )
      await removeLocalUsages(usages.map((item) => item.voucher_id))
      await refreshPending()
      if (data.rejected)
        toast(`${data.rejected} uso(s) offline já estavam usados ou expirados.`, 'warning')
    } catch {
      /* permanece na fila para a próxima tentativa */
    }
  ...
  useEffect(() => {
    const onOnline = () => {
      setOnline(true)
      fetchAndCachePublicKey(api).catch(() => {})
      syncPending()
    }
  ...
  const validate = useCallback(
    async (rawData) => {
      ...
      try {
        ...
        if (await checkLocalUsage(parsed.id))
          throw new Error('Este voucher já foi registrado neste dispositivo')
        const validSignature = await verifyVoucherSignature(
          getCachedPublicKey(),
          parsed,
          navigator.onLine ? api : undefined
        )
        ...
        if (navigator.onLine) {
          try {
            const { data } = await voucherAPI.validate({
              voucher_id: parsed.id,
              nonce: parsed.n,
              signature_hex: parsed.s,
            })
        ...
        toast(
          storedOffline
            ? 'Voucher validado e salvo para sincronizar'
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-ru-blue">
            Balcão · entrada gratuita
          </p>
          <h1 className="font-display text-3xl md:text-4xl font-bold mt-2">Validar voucher</h1>
          <p className="text-ru-muted mt-2">Confira o ID do estudante e leia o QR apresentado.</p>
        </div>
        <div
          className={`tag ${online ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}
        >
        ...
        <section className="card">
          ...
          <div className="mt-6 pt-6 border-t border-ru-cream-dark">
            ...
            <p className="text-xs text-ru-muted mt-1">
              Use esta opção se a câmera não conseguir ler o código.
            </p>
        ...
        <aside className="space-y-4">
          {result ? (
            <section className="card border-2 border-emerald-200 bg-emerald-50/50">
              ...
              <h2 className="font-display text-2xl font-bold mt-4">Voucher válido</h2>
              <p className="text-ru-charcoal font-semibold mt-4">{result.student_name}</p>
              <p className="text-sm text-ru-muted mt-1">Válido até {formatDate(result.e)}</p>
              <div className="mt-5 py-3 px-4 rounded-xl bg-white text-sm flex gap-2">
                <ShieldCheck size={18} className="text-ru-blue shrink-0" />
                {result.offline
                  ? 'Assinatura verificada offline. O uso será sincronizado.'
                  : 'Assinatura e uso confirmados no servidor.'}
              </div>
            </section>
          ) : (
            <section className="card min-h-56 grid place-items-center text-center">
              <div>
                <ShieldCheck size={34} className="mx-auto text-ru-muted" />
                <h2 className="font-display font-semibold mt-3">Aguardando leitura</h2>
                <p className="text-sm text-ru-muted mt-2">
                  O resultado da verificação aparecerá aqui.
                </p>
              </div>
            </section>
          )}
          <section className="card">
            <div className="flex items-center justify-between text-sm">
              <span className="text-ru-muted">Usos nesta sessão</span>
              <strong>{usesToday}</strong>
            </div>
            <div className="flex items-center justify-between text-sm mt-3">
              <span className="text-ru-muted">Pendentes de sincronização</span>
              <strong>{pending}</strong>
            </div>
            {pending > 0 && online && (
              <button
                className="btn-secondary w-full mt-5 inline-flex justify-center items-center gap-2"
                onClick={syncPending}
              >
