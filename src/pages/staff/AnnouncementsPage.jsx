...
export default function AnnouncementsPage() {
  ...
  const save = async () => {
    if (!form.title.trim() || !form.message.trim()) {
      toast('Preencha o assunto e o corpo do aviso', 'error')
      return
    }
    if (form.send_now && !editing) {
      const confirmed = window.confirm(
        'Enviar este aviso por e-mail para todos os usuários cadastrados agora?'
      )
      if (!confirmed) return
    }
    setSaving(true)
    ...
    try {
      ...
      saved = editing ? editing.id : data.data?.id
    } catch (error) {
      toast(
        error.response?.data?.detail?.msg || error.response?.data?.msg || 'Erro ao salvar aviso',
        'error'
      )
      return
    } finally {
    ...
    if (form.send_now && saved) {
      try {
        await announcementAPI.notify(saved)
        toast('Aviso publicado e enviado por e-mail aos cadastrados!', 'success')
      } catch {
        toast(
          'Aviso salvo, mas o envio por e-mail falhou. Use "Enviar e-mail" na lista.',
          'warning'
        )
      }
    } else if (!form.send_now) {
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-ru-muted mt-2">
            Publique avisos e dispare um e-mail para todos os cadastrados — útil para imprevistos
            como término da comida ou fechamento do RU.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <section className="card text-center py-16">
          ...
          <p className="text-ru-muted text-sm mt-2">
            Clique em "Criar aviso" para redigir o assunto e o corpo do comunicado.
          </p>
        </section>
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`tag ${a.priority === 'urgente' ? 'bg-red-50 text-red-600' : 'bg-ru-cream text-ru-muted'}`}
                    >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    onClick={() => notify(a.id)}
                    disabled={sending === a.id || !a.is_active}
                  >
                  ...
                  <button
                    className="btn-danger px-3 py-2 text-xs inline-flex items-center gap-1.5"
                    onClick={() => remove(a)}
                  >
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ex.: RU fechado hoje à noite"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-ru-charcoal mb-1">
              ...
              <textarea
                ...
                placeholder="Detalhe a mensagem que será enviada aos usuários..."
              />
            </div>
            <div className="flex items-center gap-4">
              ...
              {!editing && (
                <label className="flex-1 flex items-center gap-2 mt-6 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    className="w-4 h-4 accent-ru-blue"
                    checked={form.send_now}
                    onChange={(e) => setForm({ ...form, send_now: e.target.checked })}
                  />
                  <span className="text-sm text-ru-charcoal inline-flex items-center gap-1.5">
                    <Mail size={15} /> Enviar por e-mail agora
                  </span>
                </label>
              )}
            </div>
