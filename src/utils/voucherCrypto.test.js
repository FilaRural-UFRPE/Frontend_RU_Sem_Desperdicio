...
describe('event voucher QR v2', () => {
  it('accepts a complete v2 payload', () => {
    const voucher = { v: 2, id: 42, c: '52998224725', n: 'a'.repeat(32), s: 'b'.repeat(128), e: '2026-09-01T00:00:00Z' }
  ...
  it('verifies the signed CPF, nonce and expiry', async () => {
    ...
    const message = new TextEncoder().encode(buildValidationPayload(voucher.c, voucher.n, voucher.e))
    voucher.s = Array.from(await signAsync(message, PRIVATE_KEY), (byte) => byte.toString(16).padStart(2, '0')).join('')
